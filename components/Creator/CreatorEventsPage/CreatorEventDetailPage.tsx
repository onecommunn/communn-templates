            ...
            <div className="md:col-span-2">
              ...
              <ul className="space-y-2 text-black font-semibold text-[16px] list-disc ml-6">
                <li className="font-semibold text-[16px] ">
                  {`${formatDate(eventData?.availability[0]?.day)} - ${formatDate(eventData?.availability[eventData?.availability.length - 1]?.day)}`}
                </li>
                <li>{`${formatTime(times.startTime)} - ${formatTime(times.endTime)}`}</li>
                ...
                {isSoldOut ? (
                  ...
                  <>
                    {eventData?.guestApproval ? (
                      <Button
                        className={`w-full ${!isFormValid || isLoading ? "cursor-not-allowed" : "cursor-pointer"}`}
...
export default CreatorEventDetailPage;
